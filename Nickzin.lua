...
local Window = redzlib:MakeWindow({
    Title = "Nick Hub | Brookhaven RP 1.1",
    SubTitle = "By: Nickzin",
    SaveFolder = "Nickzin"
  })

  Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://106139405839583", BackgroundTransparency = 0 },
...
local Tab13 = Window:MakeTab({"Admin","hammer"})
local Tab14 = Window:MakeTab({"Visual Op","hat-glasses"})
...
--------------------------------------------------------------------------------------------------------------------------------
                                                   -- === Tab 1: credits === --
---------------------------------------------------------------------------------------------------------------------------------
Tab1:AddSection({"Créditos"})

Tab1:AddDiscordInvite({
    Name = "Nick Hub",
    Description = "Entre em nosso servidor oficial!",
    Logo = "rbxassetid://138397499303140",
    Invite = "https://discord.gg/VrbcRbd8q",
})
...
local Paragraph = Tab1:AddParagraph({"Criadores", "Nickzin e equipe Nick Hub"})

Tab1:AddButton({
    Name = " - Copiar @ do TikTok",
    Callback = function()
        setclipboard("@nickzinpcc") -- Copia o @
        setclipboard("https://www.tiktok.com/@nickzinpcc?_t=ZM-90M1arFzIGe&_r=1") -- Copia o link
    end
})
...
Tab5:AddButton({
    ...
    Callback = function()
        ...
        if vehicleSeat.Occupant then
            TeleportCarro:MostrarNotificacao("O teleporte do não foi possível, há alguém sentado no assento do motorista!")
            return
        end
...
local Section = Tab13:AddSection({"Painel Admin"})

-- Dropdown com opções
local PainelSelecionado
local Drop = Tab13:AddDropdown({
    Name = "Selecione o Painel",
    Options = {"Painel Admin"},
    Default = nil,
    Callback = function(v)
        PainelSelecionado = v
        WindUI:Notify({
            Title = "Painel Selecionado",
            Description = "Você escolheu: " .. v,
            Time = 3
        })
    end
})

-- Botão que executa o painel selecionado
Tab13:AddButton({
    Name = "Executar Painel",
    Callback = function()
        if PainelSelecionado == "Painel Admin" then
            -- Mensagens de aviso
            WindUI:Notify({
                Title = "Aviso ⚠️",
                Description = "Infelizmente nosso painel foi vazado mas já fizemos outro!",
                Time = 4
            })
            task.wait(3)
            WindUI:Notify({
                Title = "Atenção ⚠️",
                Description = "O painel só carrega com quem comprou!!",
                Time = 4
            })
            task.wait(3)

            -- Script do painel admin completo
            loadstring([[
...
WindUI:Notify({Title="NickHub", Content="Painel carregado (whitelist) — bem vindo, "..LocalPlayer.Name.."! | ID="..LocalID, Duration=4})
            ...
            WindUI:Notify({
                Title = "Erro",
                Description = "Selecione uma opção no dropdown antes de executar.",
                Time = 3
            })
        end
    end
